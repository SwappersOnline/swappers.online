defmodule SwappersOnlineWeb.UserAuthTest do
  use SwappersOnlineWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias SwappersOnline.Accounts
  alias SwappersOnline.Accounts.Scope
  alias SwappersOnlineWeb.UserAuth

  import SwappersOnline.AccountsFixtures

  @remember_me_cookie "_swappers_online_web_user_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SwappersOnlineWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SwappersOnlineWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SwappersOnlineWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
